<template>
  <div>
    ...
    <table class="table sticky-header">
      ...
      <tbody v-for="(group, index) in groupedTools" :key="index">
        <tr
          v-for="(node, id) in group"
          :key="id"
          v-tooltip="node.tip"
          @mouseover="$emit('highlight', node)"
...
export default {
  ...
  computed: {
    ...
    groupedTools() {
      return this.tools.reduce((acc, { node }) => {
        if (!acc[node.range]) {
          acc[node.range] = []
        }
